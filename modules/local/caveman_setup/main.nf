process cavemanSetup {
    ...
    publishDir "${params.outdir}/caveman_out/${meta.sample_id}", pattern: 'alg_bean'
    publishDir "${params.outdir}/caveman_out/${meta.sample_id}", pattern: 'caveman.cfg.ini'
    ...
    output:
    tuple val(meta), path(bam), path(bai), path(bam_match), path(bai_match), path(sample_cn_file), path(match_cn_file), path('alg_bean'), path('caveman.cfg.ini')
